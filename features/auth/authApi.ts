...
export const authApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    ...
    setUserActive: builder.mutation<User, { userId: string; active: boolean }>({
      ...
      onQueryStarted: async ({ userId, active }, { getState, queryFulfilled }) => {
        try {
          await queryFulfilled;
          logAdminAction(getState, active ? "user.activated" : "user.deactivated", "profiles", userId, undefined, { active });
        } catch {
