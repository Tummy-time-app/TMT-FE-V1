...
type Props = { vendor: Vendor; skeleton?: false } | { skeleton: true; vendor?: undefined };
...
export default function VendorCard({ vendor, skeleton }: Props) {
  ...
  const { id, name, cuisine, rating, reviewCount,
          deliveryTime, deliveryFee, minOrder, priceRange,
          isOpen, isNew, promoLabel } = vendor!;

  const priceStr = Array.from({ length: priceRange }, () => "₦").join("");

  return (
    <Link href={`/vendors/${id}`} className={`vc-card ${!isOpen ? "vc-card--closed" : ""}`}>
      {/* ── image area ── */}
      <div className="vc-img-wrap">
        ...
        <div className="vc-badges">
          ...
          {promoLabel && <span className="vc-badge vc-badge--promo">{promoLabel}</span>}
        </div>
