...
import { Wallet, TrendingUp, ArrowUpRight } from "@/components/icons";
...
export default function VendorPayoutsPage() {
  ...
  } = useGetVendorOrdersQuery(vendorId, { skip: !vendorId });
  const { data: withdrawals, isLoading: withdrawalsLoading } = useGetWithdrawalsQuery(actorId, { skip: !actorId });

  const isLoading = ordersLoading || withdrawalsLoading;
      ...
      {ordersError ? (
        ...
        <>
          <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
            <StatCard label="Available balance" value={isLoading ? "…" : formatNaira(available)} icon={Wallet} accent="success" />
            <StatCard label="Total revenue" value={isLoading ? "…" : formatNaira(revenue)} icon={TrendingUp} />
            <StatCard label="Withdrawn" value={isLoading ? "…" : formatNaira(withdrawn)} icon={ArrowUpRight} />
          </div>
