...
export default function VendorPromotionsPage() {
  ...
  const { data: promotions, isLoading, error, refetch } = useGetVendorPromotionsQuery(vendorId, { skip: !vendorId });
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-small text-text-muted">Create promo codes just for your store.</p>
        </div>
        <button
          type="button"
      ...
      <div className="mt-6 space-y-2">
        {isLoading ? (
          ...
          <EmptyState icon={Tag} title="No promotions yet" description="Create your first promo code to attract more orders." />
        ) : (
            ...
            <div key={promo.id} className="flex items-center gap-4 rounded-lg border border-border bg-surface p-4">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-caption text-text-subtle">
                  {promo.description} · Used {promo.timesUsed}
